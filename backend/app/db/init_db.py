        ...
        await conn.execute(
            text(
                ...
                SET workflow_status = CASE
                    -- Preserve the colleague's Architect/Admin workflow as a
                    -- separate compatibility path. Its legacy `status` column
                    -- remains authoritative and must not be converted into an
                    -- AEE queue item.
                    WHEN architect_id IS NOT NULL AND field_submitter_id IS NULL THEN 'AI_DETECTED'

...
ROAD_BUFFER_SQL = "ST_Buffer(asset.road_geom::geography, :corridor_m)::geometry"
ROAD_BUILDING_BUFFER_SQL = "ST_Buffer(r.geom::geography, :corridor_m)::geometry"
    ...
    asset_rows = (
        await db.execute(
            text(
                ...
                "         f.attributes, f.geom, r.geom AS road_geom, "
                ...
                "       ST_AsGeoJSON(CASE "
                "         WHEN GeometryType(asset.geom) IN ('LINESTRING', 'MULTILINESTRING') "
                f"           THEN ST_Multi(ST_CollectionExtract(ST_Intersection(asset.geom, {ROAD_BUFFER_SQL}), 2)) "
                "         WHEN GeometryType(asset.geom) IN ('POLYGON', 'MULTIPOLYGON') "
                f"           THEN ST_Multi(ST_CollectionExtract(ST_Intersection(asset.geom, {ROAD_BUFFER_SQL}), 3)) "
                "         ELSE asset.geom "
                "       END)::text AS geom_json, status.color::text AS audit_color "
    ...
    building_rows = (
        await db.execute(
            text(
                ...
                "       ST_AsGeoJSON("
                f"         ST_Multi(ST_CollectionExtract(ST_Intersection(building.geom, {ROAD_BUILDING_BUFFER_SQL}), 3))"
                "       )::text AS geom_json, a.color::text AS audit_color "
                "FROM spatial_anomalies a "
                ...
                "  AND a.status IN ('open', 'reviewing') "
                "  AND ST_DWithin(building.geom::geography, r.geom::geography, :corridor_m) "
                "  AND EXISTS ( "

...
function LayerReviewCard({
  ...
  return (
    <article className={`lr-layer-card lr-layer-card--${confidenceTone}`}>
      ...
      <button type="button" className="lr-secondary-btn" disabled={busy || readOnly} onClick={onSave}>
        {readOnly ? "MLA read-only" : busy ? "Saving…" : layer.review_status === "confirmed" ? "Save changes" : "Confirm layer"}
      </button>
    </article>
  );
}
...
export function LayerReviewView() {
  ...
  async function confirmAll() {
    if (!datasetId || !manifest || readOnly) return;
  ...
  return (
    <div className="layer-review-page" data-testid="layer-review-page">
      ...
      {readOnly && <div className="lr-manifest-warnings">MLA access is read-only. You can inspect layers, dashboards, maps and exports, but you cannot change layer classifications.</div>}
      ...
      {!datasetId ? (
        ...
        <>
          ...
          <section className="lr-review-actions">
            ...
            <div>
              <button type="button" className="lr-secondary-btn" disabled={confirmingAll || readOnly} onClick={() => void confirmAll()}>{readOnly ? "MLA read-only" : confirmingAll ? "Confirming…" : "Confirm all detected layers"}</button>

...
import {
  ...
  extensionOf,
  guessWardFromFilename,
  collectDroppedFolder,
  collectPickedFolder,
  validateShapefileBundle,
  zipCollectedFiles,
  classifyAndZipFolder,
  collectObjCompanionsFromDisk,
  FolderScanLimitError,
  type WebkitEntry,
} from "../lib/datasetFileIntake";
...
export function DatasetsView() {
  ...
  const { consumePendingFiles } = useUploadTransfer();

  useEffect(() => {
    setMounted(true);
  }, []);

  // A drag dropped anywhere on the Map page redirects here with the file(s)
  // staged in UploadTransferContext (see MapView.tsx) — pick them up once,
  // on mount, exactly the same way a local drop/browse would.
  useEffect(() => {
    const pending = consumePendingFiles();
    if (!pending || pending.length === 0) return;
    void pickFiles(pending);
    setUploadNotice(
      pending.length === 1
        ? `"${pending[0].name}" was added from the Map page — review the details below and upload.`
        : `${pending.length} files were added from the Map page — review the details below and upload.`
    );
    window.setTimeout(() => setUploadNotice(null), 6000);
    // Intentionally mount-only: pending files are a one-shot handoff.
  ...
  async function pickFolder(name: string, collectFiles: () => Promise<{ path: string; file: File }[]>) {
    ...
    try {
      ...
      const { file: zipped, isObjModel, hasGeoMeta } = await classifyAndZipFolder(name, files);
      pickFile(zipped);
      if (isObjModel) {
        setObjAutoNotice(
          hasGeoMeta

import { apiAssetUrl, apiDownload, apiGet, apiPost, apiPostForm, apiPutForm } from "./api";
...
export function fetchPointVerificationById(verificationId: string, signal?: AbortSignal) {
  return apiGet<PointVerificationRecord>(
    `/api/v1/point-verifications/workflow/by-id/${encodeURIComponent(verificationId)}`,
    signal,
  );
}
...
export function fetchAeTasks(signal?: AbortSignal) {
  return apiGet<WorkflowDashboardItem[]>("/api/v1/point-verifications/tasks", signal);
}

export function fetchAeeActivity(signal?: AbortSignal) {
  return apiGet<WorkflowDashboardItem[]>("/api/v1/point-verifications/activity", signal);
}
...
// ---------------------------------------------------------------------------
// Preserved colleague workflow: Architect -> Admin
// ---------------------------------------------------------------------------
export type LegacyPointVerificationStatus = "open" | "pending_admin" | "rejected" | "resolved";
export type VerifiedCondition = "bad" | "moderate" | "good";

export interface LegacyPointVerificationRecord {
  id: string | null;
  feature_id: string;
  dataset_id: string;
  dataset_name: string;
  ...
  status: LegacyPointVerificationStatus | null;
...
export interface LegacyArchitectSubmissionInput extends AiVerificationRequestContext {
  issueSummary: string;
  workCompleted: string;
...
export interface LegacyAdminDecisionInput extends AiVerificationRequestContext {
  decision: "approve" | "reject";
  verifiedCondition: VerifiedCondition;
  remarks: string;
}

export function fetchLegacyPointVerification(
  featureId: string,
  ai: AiVerificationRequestContext,
  signal?: AbortSignal,
) {
  const query = new URLSearchParams({ anomaly_id: ai.anomalyId, detection_mode: ai.detectionMode });
  return apiGet<LegacyPointVerificationRecord>(
    `/api/v1/point-verifications/${encodeURIComponent(featureId)}?${query.toString()}`,
    signal,
  );
}

export function fetchLegacyPointVerificationById(verificationId: string, signal?: AbortSignal) {
  return apiGet<LegacyPointVerificationRecord>(
    `/api/v1/point-verifications/legacy/by-id/${encodeURIComponent(verificationId)}`,
    signal,
  );
}

export function submitLegacyArchitectRemediation(featureId: string, input: LegacyArchitectSubmissionInput) {
  const form = new FormData();
  ...
  return apiPostForm<LegacyPointVerificationRecord>(
    `/api/v1/point-verifications/${encodeURIComponent(featureId)}/architect-submit`,
    form,
  );
}

export function submitLegacyAdminDecision(featureId: string, input: LegacyAdminDecisionInput) {
  return apiPost<LegacyPointVerificationRecord>(
    `/api/v1/point-verifications/${encodeURIComponent(featureId)}/admin-decision`,
    {
      anomaly_id: input.anomalyId,
      detection_mode: input.detectionMode,
      decision: input.decision,
      verified_condition: input.verifiedCondition,
